...
# with open("endcodelist_contact_model1.pkl", "rb") as file:
#     endcodelist = pickle.load(file)
# with open("level_score_contact_model1.pkl", "rb") as file:
#     endcode_ratio_level = pickle.load(file)
# with open("score_range_1_contact_model1.pkl", "rb") as file:
...
# df_tr = df_tr.drop(['Y'], axis = 1)
# ##################################################################################################################

# print('data preparation: i love big booooobs')
# features = [col for col in df_tr.columns if col not in ['ID_SAS', CONF.contact_y, 'market_time']]
...
#         #"max_depth" : -1,
#         "seed": 5271,
#         "n_estimators": trial.suggest_int("n_estimators", 1000,80000), 
#         "learning_rate": trial.suggest_float("learning_rate", 0.00005, 0.015), 
...
#     oof = df_tr[['ID_SAS', 'CONTACT_Y']].copy()
#     oof['predict'] = 0
#     model = [0]*fold
...
#     for fold, (trn_idx, val_idx) in enumerate(cv.split(df_tr, df_tr['CONTACT_Y'])):
        
#         X_train, y_train = df_tr.iloc[trn_idx][features], df_tr.iloc[trn_idx]['CONTACT_Y']   
#         X_valid, y_valid = df_tr.iloc[val_idx][features], df_tr.iloc[val_idx]['CONTACT_Y']
...
#     cv_scores = roc_auc_score(oof['CONTACT_Y'], oof['predict'])
...
# study = optuna.create_study(direction="maximize", study_name="boooobs", sampler=TPESampler(seed=1217))
...
# joblib.dump(study, "optuna tune/study_model1_con.pkl")
...
# with open("endcodelist_contact_model4.pkl", "rb") as file:
#     endcodelist = pickle.load(file)
# with open("level_score_contact_model4.pkl", "rb") as file:
#     endcode_ratio_level = pickle.load(file)
# with open("score_range_1_contact_model4.pkl", "rb") as file:
#     endcode_ratio = pickle.load(file)
...
# print('data preparation: i love big butt and i can not lie')
# features = [col for col in df_tr.columns if col not in ['ID_SAS', CONF.contact_y, 'market_time']]
...
# def print_best_callback(study, trial):
#     print(f"Best value: {study.best_value}, Best params: {study.best_trial.params}")
# study.optimize(objective2, n_trials=87, n_jobs=2, callbacks=[print_best_callback, lambda study, trial: gc.collect()])
...
# joblib.dump(study, "optuna tune/study_model4_con.pkl")

# del df_tr, features, study, time1, objective2
# gc.collect()
...
with open("endcodelist_contact_model2.pkl", "rb") as file:
    endcodelist = pickle.load(file)
with open("level_score_contact_model2.pkl", "rb") as file:
    endcode_ratio_level = pickle.load(file)
with open("score_range_1_contact_model2.pkl", "rb") as file:
    endcode_ratio = pickle.load(file)
...
print('data preparation: cum into pussy')
features = [col for col in df_tr.columns if col not in ['ID_SAS', CONF.contact_y, 'market_time']]
...
def print_best_callback(study, trial):
    print(f"Best value: {study.best_value}, Best params: {study.best_trial.params}")
study.optimize(objective3, n_trials=87, n_jobs=2, callbacks=[print_best_callback, lambda study, trial: gc.collect()])
...
joblib.dump(study, "optuna tune/study_model2_con.pkl")

del df_tr, features, study, time1, objective3
gc.collect()
